...
export default function ThumbnailFile({src, file, removeHandler}) {
    ...
    switch (mediaType) {
        ...
        case 'audio':
            ...
            break;
        case 'image':
            thumbnailContent = (<Image src={src} alt='File Thumbnail' className='absolute z-10 top-0 left-0 filter brightness-75' layout='fill' objectFit='cover'/>);
    }
    ...
    return (
        <div className='w-[130px] h-[80px] relative'>
            ...
            <span className='flex items-baseline gap-[2px] z-20 absolute text-xs bottom-0 left-0 p-1 text-white bg-gray-600'>{shortenedFileName}<span className='font-semibold text-sm'>&#124;</span>{format.toUpperCase()}</span>
            {thumbnailContent}
        </div>
    )
}
