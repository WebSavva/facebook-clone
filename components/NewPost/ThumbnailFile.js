    ...
    case "audio":
      thumbnailContent = (
        <Image
          src={audioImageSrc}
          alt="File Thumbnail"
          className="absolute z-10 top-0 left-0 filter brightness-75"
          layout="fill"
          objectFit="cover"
        />
      );
  ...
  return (
    <div className="w-[130px] h-[80px] relative">
      <button
        ...
        className="z-20 text-xs flex items-center justify-between text-white absolute top-0 right-0 p-1 bg-gray-600"
      >
        &#10006;
      </button>
      <span className="flex items-baseline gap-[2px] z-20 absolute text-xs bottom-0 left-0 p-1 text-white bg-gray-600">
        {shortenedFileName}
